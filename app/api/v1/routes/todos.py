...
async def create_new(todo_in: TodoCreate, db: AsyncSession = Depends(get_session)):
...
async def get_todo_by_id(todo_id: int, db: AsyncSession = Depends(get_session)):
...
async def update_todo_by_id(
    todo_id: int, todo_in: TodoUpdate, db: AsyncSession = Depends(get_session)
):
    todo = await get_todo(db, todo_id)
    if todo is None:
        raise HTTPException(status_code=404, detail="Todo not found")
...
async def delete_todo_by_id(todo_id: int, db: AsyncSession = Depends(get_session)):
    todo = await get_todo(db, todo_id)
    if todo is None:
        raise HTTPException(status_code=404, detail="Todo not found")
